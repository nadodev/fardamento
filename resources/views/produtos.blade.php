        ...
        <div class="container mx-auto px-4 relative z-10">
            <div class="text-center max-w-4xl mx-auto">
                ...
                <h1 class="text-5xl lg:text-7xl font-extrabold leading-tight mb-6">
                    Nossos <span class="bg-gradient-to-r from-orange-400 to-orange-500 bg-clip-text text-transparent">Produtos</span>
                </h1>
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="mb-12">
                <div class="flex flex-wrap items-center justify-center gap-4">
                    <button onclick="filterProducts('all')" class="filter-btn active px-6 py-3 rounded-xl bg-gradient-to-r from-blue-700 to-blue-800 text-white font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105" data-filter="all">
                        Todas as Lojas
                    </button>
                    <button onclick="filterProducts('matriz')" class="filter-btn px-6 py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all transform hover:scale-105" data-filter="matriz">
                        Loja Matriz
                    </button>
                    <button onclick="filterProducts('filial')" class="filter-btn px-6 py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all transform hover:scale-105" data-filter="filial">
                        Loja Filial
                    </button>
                </div>
            </div>
            ...
            <div id="products-grid" class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @forelse($produtos as $produto)
                    @php
                        ...
                        $lojas = $produto->lojas ?? [];
                    @endphp
                    <a href="{{ route('produto.detalhes', $produto->slug) }}" class="product-item group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100" data-loja="{{ implode(',', $lojas) }}">
                        <div class="h-64 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden relative">
                            ...
                            <div class="absolute top-3 right-3 flex gap-2">
                                @if(in_array('matriz', $lojas))
                                    <span class="px-2 py-1 bg-blue-700 text-white text-xs font-semibold rounded-lg">Matriz</span>
                                @endif
                                @if(in_array('filial', $lojas))
                                    <span class="px-2 py-1 bg-orange-600 text-white text-xs font-semibold rounded-lg">Filial</span>
                                @endif
                            </div>
                        </div>
                        <div class="p-6">
                            <div class="text-xs text-gray-500 mb-2">Código: {{ $produto->codigo }}</div>
                            <h3 class="text-xl font-bold text-gray-900 mb-3 group-hover:text-blue-700 transition-colors">{{ $produto->nome }}</h3>
                            <p class="text-gray-600 text-sm leading-relaxed mb-4 line-clamp-2">{{ $produto->descricao }}</p>
                            <span class="text-blue-700 font-semibold text-sm inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                                Ver Detalhes
            ...
            <div id="empty-state" class="hidden text-center py-16">
                ...
                <p class="text-gray-600">Tente selecionar outro filtro.</p>
            </div>
        </div>
    </section>
        ...
        function filterProducts(filter) {
            ...
            filterButtons.forEach(btn => {
                if (btn.dataset.filter === filter) {
                    ...
                    btn.classList.add('bg-gradient-to-r', 'from-blue-700', 'to-blue-800', 'text-white', 'shadow-lg', 'hover:shadow-xl');
                    btn.classList.add('active');
                } else {
                    btn.classList.remove('bg-gradient-to-r', 'from-blue-700', 'to-blue-800', 'text-white', 'shadow-lg', 'hover:shadow-xl', 'active');
